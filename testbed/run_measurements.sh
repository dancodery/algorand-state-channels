...
timestamp_start=$(echo "$runtime_recording" | awk -F '[,: ]+' '/timestamp_start/{for (i=0; i<=NF; i++) print $i}')

# timestamp_end=$(echo "$runtime_recording" | awk -F '[,: ]+' '/timestamp_end/{print $3 "." $4}')
timestamp_end=$(echo "$runtime_recording" | awk -F '[,: ]+' '/timestamp_end/{for (i=0; i<=NF; i++) print $i}')
